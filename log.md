...
Now to interpret that text so that it appears correctly in the preview. FCC reccomends [Marked](https://cdnjs.com/libraries/marked) for this. Added Marked to the pen header and got it working, which turned out to be very easy, but now the input variable is a string of HTML that renders as such a string. I need the page to parse it as actual HTML. I found that dangerouslySetInnerHTML worked although I'm not sure it was the best idea, but so far I have a working interpreter. I want to do some more styling and make it mobile friendly too, but the basic functionality is all there.

**Thoughts:** I worried that doing a single component was not the "correct" way to put the app together, but I suppose the important part is to get it done and not worry about how. I can look up alternate solutions later and see how I did. For now, this is new to me and I'll focus on making the thing work.

Happy with how easy Marked was to use. I'm hoping after I'm done with this that I can dig up better ways to parse that HTML than dangerouslySetInnerHTML. The name says it all, really.
...
**Thoughts:** Hopefully adding database functionality should be easy by now. It felt quite smooth and routine, the only difficult upcoming part I can see is working out how to get more than one result for the recent search endpoint. But I hardly think that's a problem with no solutions!
...
Got a key and hooked up the app to the API. Authentication seems to go ok, but I'm having trouble getting search results, so need to read some more documentation.
...
Now that the server can handle URL checks, it needs to interact with the database. First off, I want to pass valid URLs to the database for storage and assign them an ID that can be used to return the URL later. Managed to get the URL to save (with a dummy ID at the moment, another issue I will return to) but having trouble with getting the function to then return the JSON so the server can display it. The callback fires ok (tested with a console.log) but returning JSON alone doesn't seem to be working. Passed the response object to the controller and put the send method in the save callback, and that works.
...
I am very pleased with how much functionality I managed to add today. Next up will be to generate unique ID numbers for each URL so that they can be reused to return the URL on the other endpoint.
